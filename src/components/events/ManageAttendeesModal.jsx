...
const ManageAttendeesModal = ({ open, onClose, event, onSuccess }) => {
    ...
    const handleRemoveAttendee = async (attendee) => {
        if (
            !window.confirm(
                `Are you sure you want to remove ${attendee.firstName} ${attendee.lastName}?`
            )
        )
            return;
        ...
        try {
            ...
            await unregisterFromEvent(
                event._id || event.id,
                attendee._id || attendee.memberId
            );
    ...
    const availableMembers = members.filter(
        (m) => !attendees.some((a) => a.email === m.email || a._id === m._id)
    );
                ...
                <Box sx={{ mb: 3, display: 'flex', gap: 1 }}>
                    <Autocomplete
                        ...
                        getOptionLabel={(option) =>
                            `${option.firstName || option.first} ${option.lastName || option.last} (${option.email})`
                        }
