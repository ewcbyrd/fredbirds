...
const EventForm = ({ event, onSuccess, onCancel }) => {
    ...
    useEffect(() => {
        const fetchMembers = async () => {
            try {
                ...
                console.log(
                    'Loaded members for trip leader:',
                    members?.length || 0
                );
    ...
    useEffect(() => {
        if (event) {
            setFormData({
                ...
                isClubEvent:
                    event.isClubEvent !== undefined ? event.isClubEvent : true,
                locations:
    ...
    const validateForm = () => {
        ...
        const hasLocations = formData.locations.some(
            (loc) => loc.lat || loc.lon || loc.name || loc.address
        );
        if (hasLocations) {
            const hasValidLocation = formData.locations.some(
                (loc) =>
                    loc.lat &&
                    loc.lon &&
            ...
            if (!hasValidLocation) {
                setError(
                    'If you provide location information, at least one location must have valid coordinates (latitude and longitude)'
                );
                return false;
            }
        }
        ...
        if (formData.ebirdTripUrl.trim()) {
            try {
                ...
                if (!url.protocol.match(/^https?:$/)) {
                    setError(
                        'eBird Trip Report URL must use http or https protocol'
                    );
                    return false;
                }
            } catch (e) {
            ...
            const eventData = {
                ...
                tripLeader: formData.tripLeader
                    ? {
                          ...
                          firstName:
                              formData.tripLeader.firstName ||
                              formData.tripLeader.first ||
                              '',
                          lastName:
                              formData.tripLeader.lastName ||
                              formData.tripLeader.last ||
                              '',
                          name:
                              formData.tripLeader.name ||
                              `${formData.tripLeader.firstName || formData.tripLeader.first || ''} ${formData.tripLeader.lastName || formData.tripLeader.last || ''}`.trim()
                      }
                    : null,
            ...
            >
                ...
                <Autocomplete
                    ...
                    onChange={(event, newValue) =>
                        handleChange('tripLeader', newValue)
                    }
                ...
                <TextField
                    ...
                    onChange={(e) =>
                        handleChange('ebirdTripUrl', e.target.value)
                    }
                    margin="normal"
                ...
                <FormControlLabel
                    control={
                        <Checkbox
                            ...
                            onChange={(e) =>
                                handleChange('isClubEvent', e.target.checked)
                            }
                        />
                    }
                ...
                <FormControlLabel
                    control={
                        <Checkbox
                            ...
                            onChange={(e) =>
                                handleChange('cancelled', e.target.checked)
                            }
                        />
                    }
