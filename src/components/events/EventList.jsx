...
const EventList = ({ onEditEvent, refreshTrigger }) => {
    ...
    const filterEvents = () => {
        ...
        if (statusFilter === 'active') {
            filtered = filtered.filter(
                (event) => !event.cancelled && new Date(event.start) >= now
            );
                            ...
                            <CardContent>
                                ...
                                {event.tripLeader && (
                                    ...
                                    >
                                        ...
                                        >
                                            Led by{' '}
                                            {event.tripLeader.name ||
                                                `${event.tripLeader.firstName} ${event.tripLeader.lastName}`.trim()}
                                        </Typography>
                                    </Box>
                                )}
                ...
                <DialogContent>
                    ...
                    {eventToDelete?.attendeeCount > 0 && (
                        <Alert severity="warning" sx={{ mt: 2 }}>
                            This event has {eventToDelete.attendeeCount}{' '}
                            registered{' '}
                            {eventToDelete.attendeeCount === 1
                                ? 'attendee'
                                : 'attendees'}
                            . Deleting this event will remove all attendance
                            records.
                        </Alert>
                    )}
                    <Typography
