...
const LocationDialog = ({ open, onClose, event, locationIndex, onEventUpdated }) => {
    ...
    useEffect(() => {
        if (open) {
            if (isEditMode && event.locations && event.locations[locationIndex]) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if ((formData.lat && isNaN(newLocation.lat)) || (formData.lon && isNaN(newLocation.lon))) {
            ...
            const currentLocations = event.locations ? [...event.locations] : []
            ...
            const cleanedLocations = currentLocations
                .map(loc => ({
                    name: loc.name || '',
                    lat: loc.lat !== '' && !isNaN(parseFloat(loc.lat)) ? parseFloat(loc.lat) : '',
                    lon: loc.lon !== '' && !isNaN(parseFloat(loc.lon)) ? parseFloat(loc.lon) : '',
                    address: loc.address || ''
                }))
            ...
            <DialogContent dividers>
                ...
                <Box sx={{ mt: 2 }}>
                    <Typography variant="caption" color="text.secondary">
                        * Latitude and Longitude are required for the map to appear.
                    </Typography>
                </Box>
