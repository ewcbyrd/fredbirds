...
export default function Resources() {
    ...
    async function fetchRareBirds() {
        ...
        if (cached) {
            try {
                ...
                const rarebirdsSet = new Set(
                    rarebirdsArray.map((item) =>
                        item['Scientific Name']?.toLowerCase()
                    )
                );
        ...
        try {
            ...
            if (res && Array.isArray(res)) {
                const rarebirdsSet = new Set(
                    res.map((item) => item['Scientific Name']?.toLowerCase())
                );
                setRarebirds(rarebirdsSet);
    ...
    const processSightings = () => {
        ...
        speciesSet.forEach((species) => {
            const matches = rawSightings.filter(
            ...
            matches.forEach((item) => {
                if (
                    individualSightings.findIndex(
                        (temp) => temp.id === item.obsId
                    ) < 0
                ) {
    ...
    async function fetchStates() {
        ...
        try {
            ...
            const opts = Array.isArray(res)
                ? res.map((i) => ({ label: i.state, value: i.code }))
                : [];
            opts.sort((a, b) => a.label.localeCompare(b.label));
            setStateOptions(opts);
    ...
    async function fetchCounties() {
        ...
        if (cached) {
            try {
                ...
                const vaCounties = opts.filter((c) =>
                    c.value.startsWith('US-VA')
                );
        ...
        try {
            ...
            const opts = Array.isArray(res)
                ? res.map((i) => ({ label: i.county, value: i.code }))
                : [];
            opts.sort((a, b) => a.label.localeCompare(b.label));
            setCountyOptions(opts);
            // Filter for Virginia by default
    ...
    async function loadSightings() {
        ...
        try {
            if (setting.view === 'local') {
                ...
                const res = await getNearbyNotableObservations({
                    lat: 38.31,
                    long: -77.46,
                    daysBack: setting.back
                });
                setRawSightings(res || []);
                ...
                const res = await getNotableSightingsByLocation({
                    regionCode,
                    daysBack: 3
                });
                setRawSightings(res || []);
                ...
                await new Promise((resolve, reject) => {
                    navigator.geolocation.getCurrentPosition(
                        async (position) => {
                            try {
                                const res = await getNearbyObservations({
                                ...
                                console.error(
                                    'Error loading nearby sightings:',
                                    e
                                );
                                setRawSightings([]);
                                reject(e);
    ...
    const totalIndividuals = sightings
        ? sightings.reduce(
              (acc, curr) =>
                  acc +
                  (curr.individualSightings &&
                  Array.isArray(curr.individualSightings)
                      ? curr.individualSightings.length
                      : 1),
              0
          )
        : 0;
